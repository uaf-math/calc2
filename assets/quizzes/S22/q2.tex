...
\problem{7}  Find the area of the region enclosed by the curves $x = 2 - y^2$ and $x = |y|$.  (\textsl{Hint: Sketch the region first.  Which variable to use for integration?})
\vfill
...
\begin{subproblems}
\item Sketch the region bounded by $y=x^2$, $x=0$, and $y=1$.
\vfill
...
\problem{5}  Set up, but do not evaluate, an integral for the area between $y=\cos(x)$ and $y=\cos(x)^2$ on the interval $0 \le x \le \pi/2$.  (\textsl{Hint: Sketch the region first.  Which function has larger values?})
\vfill

\newpage
\nameprob{EC}{1} (\emph{Extra Credit})  Evaluate the integral in problem \textbf{3.}
\vfill
