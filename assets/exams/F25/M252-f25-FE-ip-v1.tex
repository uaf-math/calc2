...
\newpage
\begin{enumerate}
\item (6 pts) The region $R$ is bounded by $ y = \sin^3 (x) \cos^2 (x) $ and the x-axis between $x = 0$ and $ x = \pi/2$.  Find the \textbf{area} of the region $R.$
...
\begin{axis}[
    ...
    grid = major]
% Plot the curve
\addplot[<->,ultra thick, black, domain= -3.9:3.9] {(sin(deg(x))^3)*cos(deg(x))*cos(deg(x))};
\node at (1.4,1){{\Large{$y=\cos^2(x)$}}};
\end{axis}
\end{tikzpicture}
\vfill
...
%% Mass (arctan IBP)
\newpage
\item (6 pts) The shaft of a bird feather has density function \fbox{$\rho = \tfrac{2}{3}\arctan x $} grams per meter on the interval from $x=0$ m to $x=1$ m. Find the \emph{mass} of the shaft. \emph{Include units with your answer.}
\vfill
%% Integration:  IBP ; trig sub ; partial fractions ; 

\item (5 pts each) Evaluate the indefinite integrals. 
	\begin{enumerate}
	\item $\displaystyle \int \dfrac{1}{x^2\sqrt{x^2-9}} dx $
	\vfill
\newpage
	\item $\displaystyle \int \frac{1}{x^2-4} dx$
	\vfill
	\item $\displaystyle \int (3x-xe^x ) dx$
	\vfill
	\end{enumerate}
...
\item (10 pts) The region $R$ is bounded by $y= e^x$, $y=4$, and the y-axis.
\begin{enumerate}
...
\item (5 pts each) Determine whether each series below converges or diverges. Name the test you use and justify your conclusion. (This means show your work!)
	...
	\vfill
	\item $\displaystyle\sum_{n=1}^{\infty} \frac{2n+3}{7n^{3/2}+1}$\hfill Test: \underline{\hspace{2in}}  \hfill Converge or Diverge\\
...
\newpage
	...
	\vfill
	\item $\displaystyle\sum_{n=1}^{\infty} \cos\left(\frac{1}{3n^2+2}\right)$ \hfill Test: \underline{\hspace{2in}}  \hfill Converge or Diverge\\
...
\item (5 pts each) For each power series below, determine the interval of convergence.
	\begin{enumerate}
	\item $\displaystyle\sum_{n=1}^{\infty} \frac{n!(x+3)^n}{6^{2n}}$
	\vfill
	\item $\displaystyle\sum_{n=1}^{\infty} \frac{(x-4)^n}{n5^n}$
	\vfill
	\end{enumerate}
\newpage
...
\end{enumerate}
\newpage
\item (5 pts) Make a careful sketch of the polar curve $r = 1 + 2\cos(\theta)$. Label at least 4 points.\\
