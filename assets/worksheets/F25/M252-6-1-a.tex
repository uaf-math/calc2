...
\begin{center}
  \large
  \sc{Section 6.1: Power Series (Day 1)}\\
   
\end{center}

\begin{enumerate}
\item A \textbf{power series centered at $x=0$} has the form:\\

\vfill


\item A \textbf{power series centered at $x=a$} has the form:\\

\vfill

\item Some Examples
	\begin{enumerate}
	\item $\ds \sum_{n=0}^\infty (n+1)x^n$
	\vfill
	\item $\ds \sum_{n=0}^\infty \frac{(x-2)^n}{(n+1)^4}$
	\vfill
	\end{enumerate}
\newpage
\item Convergence of a Power Series.

\vfill

\item Determine the \textbf{radius} and \textbf{interval} of convergence for the power series below.
	\begin{enumerate}
	\item $\ds \sum_{n=0}^\infty (n+1)x^n$
	\vfill
	\item $\ds \sum_{n=0}^\infty \frac{(x-2)^n}{(n+1)^4}$
