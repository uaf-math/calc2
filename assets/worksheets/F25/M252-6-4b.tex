...
\begin{center}
  \large
  \sc{Section 6.4: Working with Taylor Series (Day 2)}\\
  
\end{center}

\begin{enumerate}
\item Write the Maclaurin Series for $f(x) = \sin(x)$ and determine its interval of convergence.
\vfill
\vfill
\item Use your answer in problem 1 to find the Maclaurin series for\\
	...
	\end{enumerate}
\newpage
\item Several Maclaurin Series to know:
	\begin{enumerate}
	\item $f(x)=e^x=$
	\vskip 0.7in
	\item $f(x)= \ln(1+x))=$
	...
	\vskip 0.7in
	\end{enumerate}
\item We now know the sum of the alternating harmonic series.
\end{enumerate}
\end{document}
