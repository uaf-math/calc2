...
\begin{document}
\scriptsize \noindent Math 252 Calculus II (Bueler) \hfill 7 February 2022 \fbox{\emph{Not to be turned in!}}
\normalsize\medskip

\Large\centerline{\textbf{Worksheet: Various trigonometric integrals}}
\medskip
\normalsize
...
\prob{A}  $\ds \int \tan(4x)\,dx =$
\vfill

\prob{B}  $\ds \int \sec^2 x \tan^3 x\,dx =$
\vfill

\prob{C}  $\ds \int_0^\pi \sin(4x) \cos(3x) \,dx =$
\vfill

\prob{D}  $\ds \int \tan^4 t\,dt =$
\vfill

\prob{E}  $\ds \int \sec(2x)\,dx =$
\vfill
\end{document}
