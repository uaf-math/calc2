...
\begin{document}
\scriptsize \noindent Math 252 Calculus II (Bueler) \hfill 4 February 2022 \fbox{\emph{Not to be turned in!}}
\normalsize\medskip

\Large\centerline{\textbf{Worksheet: Integrals of powers of $\sin$ and $\cos$}}
\medskip
\normalsize

\thispagestyle{empty}
\begin{center}
Compute these integrals with a group, if possible!
\end{center}

\prob{A}  $\ds \int_{\pi/4}^{\pi/3} \cos^4 x \sin x\,dx =$
\vfill

\prob{B}  $\ds \int \cos^3 x \sin^4 x\,dx =$
\vfill

\prob{C}  $\ds \int \sin^2(4x)\,dx =$
\vfill

\prob{D}  $\ds \int e^{\sin x} \cos^3 x\,dx =$
\vfill

\prob{E}  $\ds \int \sin 2x \cos x\,dx =$
\vfill
\end{document}
