...
\begin{center}
  \large
  \sc{Section 7.4: Area in Polar Coordinates}\\
   
\end{center}

\begin{enumerate}
\item Suppose $r=f(\theta)$ is a continuous and nonnegative on the interval from $\alpha \leq \theta \leq \beta$, then the area bounded by $r=f(\theta)$ and the radial lines $\theta = \alpha$ and $\theta = \beta$ is

\vfill

\item Set up and evaluate the integral to find the area enclosed by the polar curve $r=10 \cos (\theta).$
\vfill
\newpage
\item Let $R$ be the region enclosed by the polar curve $r=2 \cos (3 \theta).$ Shade the region $R$, then Set up and evaluate the integral to find the area of $R.$\\


\includegraphics[scale=0.25]{3-petal}
\end{enumerate}
\end{document}
