...
%\usepackage[top=10mm, bottom=10mm, left=0mm, right=10mm, showframe, foot=5mm, marginparsep=0mm]{geometry}
\usepackage[left=0.2in,top=0.35in,bottom=0.25in]{geometry}
...
\newcommand{\theblock}{
\begin{minipage}[t][4.9in][t]{3.85in}
\scriptsize \noindent 9 Sep 2022 \hfill \normalsize NAME: \underline{\hspace{60mm}}
\medskip

\thispagestyle{empty}

\noindent \textbf{Q.}\quad Sketch the region between the graph of {\large $y=e^{-x^2}$} and the $x$-axis, on the interval $[0,2]$.  Then find the volume of the solid from rotating the region around the $y$-axis.  (\emph{Hint}.  Which technique gives you an integral you can do?)

% solution: by shells
% V = \int_0^2 2 \pi x \cdot e^{-x^2} \cdot dx = \pi (1-e^{-4})
\vfill
\end{minipage}
}
